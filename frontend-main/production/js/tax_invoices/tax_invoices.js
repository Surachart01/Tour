...
async function loadBookings() {
  ...
  body.innerHTML = '<tr><td colspan="9" class="text-center" style="padding:40px"><i class="fa fa-spinner fa-spin"></i> Loading confirmed bookings...</td></tr>';
  try {
...
function renderBookings() {
  ...
  if (!rows.length) {
    body.innerHTML = '<tr><td colspan="9"><div class="empty-state"><i class="fa fa-file-text-o"></i><strong>No confirmed bookings found</strong><div>Tax documents become available after a booking is confirmed.</div></div></td></tr>';
    return;
  }
  body.innerHTML = rows.map((booking) => {
    const documents = booking.tax_documents || [];
    const generatedTypes = new Set(documents.map((document) => document.document_type));
    const actions = DOCUMENTS.map((document) => `
      <button class="btn btn-xs ${document.className}" data-trip-id="${booking.id}" data-document-type="${document.type}" title="${document.label}">
        <i class="fa ${document.icon}"></i> ${document.label}
      </button>`).join('');
    const generated = documents.length
      ...
      : '<span class="doc-state">No saved tax document yet</span>';
    return `<tr>
